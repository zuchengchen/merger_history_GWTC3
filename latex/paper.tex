...
\email{Corresponding author: zhiqiang.you@bnu.edu.cn}
\affiliation{Department of Astronomy, Beijing Normal University, Beijing 100875, China}
\affiliation{Advanced Institute of Natural Sciences, Beijing Normal University, Zhuhai 519087, China}
...
In practice, we use the simulated injections \cite{ligo_scientific_collaboration_and_virgo_2021_5546676} to estimate $\xi(\Ld)$, and \Eq{xi} can be approximated by a Monte Carlo integral over found injections \cite{KAGRA:2021duu}
\begin{equation}
...
Using $69$ BBHs from GWTC-3 and performing the hierarchical Bayesian inference, we obtain $\Mf = 10.8^{+3.7}_{-3.6} \Msun$, $\al = 1.0^{+0.2}_{-0.2}$, and $\fpbh = 1.5^{+0.2}_{-0.2} \times 10^{-3}$. The posteriors for the hyper-parameters $\Ld = \{\Mf, \al, \fpbh\}$ are shown in \Fig{posterior-CC}. From \Eq{Rt}, we also infer the local merger rate as $R(t_0) = 49^{+26}_{-16} \gpcyr$. The results of the local merger rate and abundance of PBHs are consistent with the previous estimations \cite{Sasaki:2016jop,Ali-Haimoud:2017rtz,Chen:2018czv,Chen:2018rzo,Chen:2019irf,Wu:2020drm,Chen:2021nxo,Chen:2022fda,Zheng:2022wqo}, confirming that CDM cannot be dominated by the stellar-mass PBHs. 
