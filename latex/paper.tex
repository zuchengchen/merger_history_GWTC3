...
The BBHs observed by LVK suggest that BHs should have a broad mass distribution, and hence we consider an extended mass function for PBHs. Here, we denote the probability distribution function (PDF) of PBH masses by $P(m)$ and demand the PDF to be normalized such that
\e\label{norm}
...
\e 
    0.85 \fpbh\, P(m)\, \rd m,
\q 
where the coefficient $0.85$ is roughly the fraction of CDM in non-relativistic matter including both CDM and baryons. Following \cite{Liu:2019rnx}, we may also define the ``averaged" PBH mass, $\mpbh$, as
\e\label{mpbh}
